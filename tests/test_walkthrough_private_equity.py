"""
Unit tests for the /walkthrough-private-equity slash command.

Validates structure, tool coverage, phases, and PE role framing.
"""
...
# MCP tools required for private equity workflow
REQUIRED_MCP_TOOLS = [
...
def read_walkthrough_file():
    """Read walkthrough-private-equity.md and return (frontmatter, body)."""
    path = COMMANDS_DIR / "walkthrough-private-equity.md"
    text = path.read_text()
...
def test_walkthrough_file_exists():
    """walkthrough-private-equity.md must exist in .claude/commands/."""
    path = COMMANDS_DIR / "walkthrough-private-equity.md"
...
def test_allowed_tools_coverage():
    """allowed-tools must list all 4 private equity MCP tools."""
    frontmatter, _ = read_walkthrough_file()
...
# ---------------------------------------------------------------------------
# Test 5: All 12 steps present
# ---------------------------------------------------------------------------


def test_has_12_steps():
    """Body must contain Steps 1 through 12."""
...
def test_role_framing():
    """Body must use private equity deal team and IC framing language."""
    _, body = read_walkthrough_file()
    body_lower = body.lower()
    pe_terms = ["deal team", "investment committee", "ic", "due diligence", "portfolio", "fund iii"]
    found = [t for t in pe_terms if t in body_lower]
    assert len(found) >= 3, (
        f"Only found {len(found)}/6 private equity framing terms: {found}"
    )
...
def test_traditional_tool_mapping():
    """Body must map Finance AI tools to traditional PE research tools."""
    _, body = read_walkthrough_file()
    body_lower = body.lower()
    assert "pitchbook" in body_lower or "preqin" in body_lower
