"""Tests for MassiveClient low-level HTTP client."""
...
from finance_mcp.providers.massive.client import MassiveAPIError, MassiveClient


class TestMassiveAPIError:
    """Tests for MassiveAPIError exception class."""

    def test_stores_status_code(self) -> None:
        err = MassiveAPIError(403, "Forbidden")
    ...
    def test_is_exception(self) -> None:
        err = MassiveAPIError(500, "Internal Server Error")
        assert isinstance(err, Exception)


class TestMassiveClientInit:
    """Tests for MassiveClient.__init__."""

    def test_init_stores_api_key(self) -> None:
        client = MassiveClient(api_key="test-key-123")
    ...
    def test_base_url_custom(self) -> None:
        client = MassiveClient(api_key="test-key", base_url="https://custom.example.com")
...
class TestMassiveClientBuildUrl:
    """Tests for MassiveClient._build_url."""
    ...
    def test_build_url_custom_base(self) -> None:
        client = MassiveClient(api_key="test-key", base_url="https://staging.polygon.io")
        url = client._build_url("/v3/reference/tickers")
        assert url == "https://staging.polygon.io/v3/reference/tickers"
        client.close()


class TestMassiveClientDefaultParams:
    """Tests for MassiveClient._default_params."""

    def test_default_params_include_apikey(self) -> None:
        client = MassiveClient(api_key="my-api-key")
...
class TestMassiveClientGet:
    ...
    def test_get_sync_403_raises(self) -> None:
        """GET raises MassiveAPIError on 403 response."""
        mock_response = MagicMock(spec=httpx.Response)
    ...
    def test_get_sync_500_raises(self) -> None:
        """GET raises MassiveAPIError on 5xx response."""
        mock_response = MagicMock(spec=httpx.Response)
    ...
    def test_get_logs_to_stderr(self, capsys: pytest.CaptureFixture) -> None:
        ...
        assert "[massive] GET /v3/reference/tickers" in captured.err
        client.close()


class TestMassiveClientClose:
    """Tests for MassiveClient.close."""
