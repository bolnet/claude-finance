"""
Unit tests for the /walkthrough-investment-banking slash command.
...
# MCP tools required for investment banking workflow
REQUIRED_MCP_TOOLS = [
...
REQUIRED_TICKERS = ["MSFT", "GOOGL", "AMZN", "CRM", "ORCL"]


def read_walkthrough_file():
    """Read walkthrough-investment-banking.md and return (frontmatter, body)."""
    path = COMMANDS_DIR / "walkthrough-investment-banking.md"
    text = path.read_text()
...
def test_walkthrough_file_exists():
    """walkthrough-investment-banking.md must exist in .claude/commands/."""
    path = COMMANDS_DIR / "walkthrough-investment-banking.md"
...
# ---------------------------------------------------------------------------
# Test 5: All 19 steps present
# ---------------------------------------------------------------------------


def test_has_19_steps():
    """Body must contain Steps 1 through 19."""
...
def test_role_framing():
    """Body must use investment banking pitch book framing language."""
    _, body = read_walkthrough_file()
    body_lower = body.lower()
    ib_terms = ["pitch book", "comps", "deal", "md", "exhibit", "valuation"]
    found = [t for t in ib_terms if t in body_lower]
    assert len(found) >= 3, (
        f"Only found {len(found)}/6 investment banking framing terms: {found}"
    )
...
def test_traditional_tool_mapping():
    ...
    _, body = read_walkthrough_file()
    body_lower = body.lower()
    assert "bloomberg" in body_lower or "capiq" in body_lower
